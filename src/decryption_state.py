...
class PartiallyDecrypted:
    ...
    def score_next_seq(self: 'PartiallyDecrypted', seq: Cipher) -> float:
        ...
        if i == -1:
            i = 0
        encrypted_by_seq: Cipher = self.encrypted[self.pos : self.pos+len(seq)]
        decryption_candidate: Optional[str] = encrypted_by_seq.xor(seq).word
        if decryption_candidate == None:
            return 0
        else:
            newly_decrypted: str = \
